...
public:
	...
	void timeStep()
	{
		...
		{
			Vec3 temp = pos;
			pos = pos + (pos-old_pos)*(1.0-DAMPING) + acceleration*g_DeltaTime;
			old_pos = temp;
...
void display(void)
{
	...
	cloth1.addForce(Vec3(0,-0.2,0)*g_DeltaTime); // add gravity each frame, pointing down
	cloth1.windForce(Vec3(0.5,0,0.2)*g_DeltaTime); // generate some wind each frame
