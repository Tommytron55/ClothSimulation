...
void main()
{
	...
	//Adding grey fog fading effect along with the light texture
	
	float d = distance(mWorldPos.xyz, cameraPos);
	float lerp = (d - 250.0f)/500.0f;
	lerp = clamp(lerp, 0.0, 1.0);
	
	vec4 vFogColour = vec4(0.5f, 0.5f, 0.5f, 1.0f);
	
	color = mix(color, vFogColour, lerp);
}